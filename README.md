...
**② 视频 URL（需 yt-dlp；部分平台需 cookie，见第六节）**
```bash
...
- **人肉手动**：照下面命令自己跑，方便排查或验证（`<skill_dir>` 含义见「零、快速开始」）。

### 5.2 确认视频源
- **本地绝对路径**：`/Users/you/视频.mp4` —— **无需 yt-dlp、无需 cookie、视频不出本机**，最省心。
- **URL**：`https://...` —— 需 `yt-dlp`；YouTube / B 站 / Dailymotion 公开链直下，抖音等需 cookie → 回退发本地文件（见第六节）。
